...
class CRNN(object):
    ...
    def train(self, iteration_count):
        # self.data_manager.generate_all_train_batches()
        with self.session.as_default():
            print("Training")
            for i in range(self.step, iteration_count + self.step):
                ...
                batch_count = 0
                iter_loss = 0
                # train_batches = self.generate_all_train_batches()
                train_batches = self.data_manager.generate_train_batches()
                for batch_y, batch_dt, batch_x in train_batches :
                    ...
                    if i % 10 == 0:
                        for j in range(batch_y.shape[0]):
                            ...
                            print("Trainset: {} | {}".format(batch_y[j],pred))
                            if pred == batch_y[j]:
                                true_numer = true_numer + 1
                        print("---- {} | {} ----".format(i, batch_count))
                        print("Train Accuary: {}".format(true_numer / len(train_batches)))
